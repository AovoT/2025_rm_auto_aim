...
ControllerIONode::ControllerIONode(const rclcpp::NodeOptions &options)
  ...
  // Subsciption
    ...
    m_tracker_sub = this->create_subscription<AimTarget>(
        "/armor_tracker", rclcpp::SensorDataQoS(),
    ...
    if (serial_msg->id == ControllerId || serial_msg->id == SlaveControllerId) {
        switch (serial_msg->func_code) {
            ...
            case GimbalPose: {
                ...
                // t.header.frame_id = serial_msg->id == ControllerId? "camera_frame": "slave_imu_frame"; // 设置父坐标系
                // t.child_frame_id = serial_msg->id == ControllerId? "imu_frame": "slave_odom_frame"; // 设置子坐标系
                // 设置平移
                ...
                t.transform.rotation = tf2::toMsg(q.inverse()); //转换为ros消息格式
